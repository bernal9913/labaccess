...
const EntryForm = ({ fetchEntries }) => {
	...
	const rooms = [
		...
		"Bioterio",
		"Sala de Cuarentena",
		"Sala de Reproducción 1",
		"Sala de Reproducción 2"
	];

	useEffect(() => {
		const fetchFrequentUsers = async () => {
			const q = collection(db, "frequentUsersD");
			try {
	...
	const handleSubmit = async (e) => {
		...
		try {
			await setDoc(doc(db, 'entriesD', id), {
				name: entryName,
				reason: entryReason,
				entryTime,
				dentro,
	...
	return (
		<Container component="main" maxWidth="xs">
			<Typography component="h1" variant="h5" align="center" sx={{ mt: 2 }}>
				Bienvenido al laboratorio DIPA
			</Typography>
